...
def test_export_new_show():
    ...
    spc.export(playlist_name=MOCK_SHOW_JSON['media_name'],
...
def test_export_new_movie():
    ...
    spc.client.reset_counter()
    spc.export(playlist_name=MOCK_MOVIE_JSON['media_name'],
...
def test_export_new_game():
    ...
    spc.client.reset_counter()
    spc.export(playlist_name=MOCK_GAME_JSON['media_name'],
...
def test_export_existing_show():
    ...
    spc.export(playlist_name=MOCK_SHOW_JSON['media_name'],
               track_uris=uris_show,
               description='')
    assert MOCK_SHOW_JSON['media_name'] in [x['name'] for x in spc.client._crt_playlists['items']], \
...
def test_export_existing_movie():
    ...
    spc.export(playlist_name=MOCK_MOVIE_JSON['media_name'],
               track_uris=uris_movie,
               description='')
    assert MOCK_MOVIE_JSON['media_name'] in [x['name'] for x in spc.client._crt_playlists['items']], \
...
def test_export_existing_game():
    ...
    spc.export(playlist_name=MOCK_GAME_JSON['media_name'],
               track_uris=uris_game,
               description='')
    assert MOCK_GAME_JSON['media_name'] in [x['name'] for x in spc.client._crt_playlists['items']], \
        f'_crt_playlists should contain game item! Instead got:  {spc.client._crt_playlists["items"]} .'
    spc.client.reset_counter()
    spc.export(playlist_name=MOCK_GAME_JSON['media_name'],
               track_uris=uris_game,
...
def test_playlist_exists_show():
    spc = SpotifyClient()
    uris_show = _get_show_uris()
    assert not spc._playlist_exists(MOCK_SHOW_JSON['media_name']), \
        f'Playlist {MOCK_SHOW_JSON["media_name"]} should not exist yet!'
    spc.export(playlist_name=MOCK_SHOW_JSON['media_name'],
               track_uris=uris_show,
               description='')
    assert spc._playlist_exists(MOCK_SHOW_JSON['media_name']), \
        f'Playlist {MOCK_SHOW_JSON["media_name"]} should exist now!'


def test_playlist_exists_movie():
    spc = SpotifyClient()
    uris_game = [x['spotify'] for x in MOCK_MOVIE_JSON['songs']]
    assert not spc._playlist_exists(MOCK_MOVIE_JSON['media_name']), \
        f'Playlist {MOCK_MOVIE_JSON["media_name"]} should not exist yet!'
    spc.export(playlist_name=MOCK_MOVIE_JSON['media_name'],
               track_uris=uris_game,
               description='')
    assert spc._playlist_exists(MOCK_MOVIE_JSON['media_name']), \
        f'Playlist {MOCK_MOVIE_JSON["media_name"]} should exist now!'


def test_playlist_exists_game():
    spc = SpotifyClient()
    uris_game = [x['spotify'] for x in MOCK_GAME_JSON['songs']]
    assert not spc._playlist_exists(MOCK_GAME_JSON['media_name']), \
        f'Playlist {MOCK_GAME_JSON["media_name"]} should not exist yet!'
    spc.export(playlist_name=MOCK_GAME_JSON['media_name'],
               track_uris=uris_game,
               description='')
    assert spc._playlist_exists(MOCK_GAME_JSON['media_name']), \
        f'Playlist {MOCK_GAME_JSON["media_name"]} should exist now!'
...
def test_get_playlist_id_show():
    ...
    spc.export(playlist_name=MOCK_SHOW_JSON['media_name'],
               track_uris=uris_show,
               description='')
    assert spc._get_playlist_id(MOCK_SHOW_JSON['media_name']) == 12, \
        f'Expected id for media {MOCK_SHOW_JSON["media_name"]} to be 12!'
...
def test_get_playlist_id_movie():
    ...
    spc.export(playlist_name=MOCK_MOVIE_JSON['media_name'],
               track_uris=uris_movie,
               description='')
    assert spc._get_playlist_id(MOCK_MOVIE_JSON['media_name']) == 34, \
        f'Expected id for media {MOCK_MOVIE_JSON["media_name"]} to be 34!'
...
def test_get_playlist_id_game():
    ...
    spc.export(playlist_name=MOCK_GAME_JSON['media_name'],
               track_uris=uris_game,
               description='')
    assert spc._get_playlist_id(MOCK_GAME_JSON['media_name']) == 56, \
        f'Expected id for media {MOCK_GAME_JSON["media_name"]} to be 56!'


def test_get_playlist_id_non_existent():
    spc = SpotifyClient()
    assert spc._get_playlist_id(MOCK_GAME_JSON['media_name']) == '', \
        f'Expected id for media {MOCK_GAME_JSON["media_name"]} to be empty!'
...
def test_item_exists_in_playlist_show():
    ...
    for uri in uris_show:
        assert spc._item_exists_in_playlist(playlist_id=spc._get_playlist_id(MOCK_SHOW_JSON['media_name']),
                                            track_uri=uri)
...
def test_item_exists_in_playlist_movie():
    ...
    for uri in uris_movie:
        assert spc._item_exists_in_playlist(playlist_id=spc._get_playlist_id(MOCK_MOVIE_JSON['media_name']),
                                            track_uri=uri)
...
def test_item_exists_in_playlist_game():
    ...
    for uri in uris_game:
        assert spc._item_exists_in_playlist(playlist_id=spc._get_playlist_id(MOCK_GAME_JSON['media_name']),
                                            track_uri=uri)
...
def test_item_exists_in_playlist_non_existent():
    ...
    for uri in ['as9f8h9ß', 'adza8snr', 'g7asencg']:
        assert not spc._item_exists_in_playlist(playlist_id=spc._get_playlist_id(MOCK_GAME_JSON['media_name']),
                                                track_uri=uri)
